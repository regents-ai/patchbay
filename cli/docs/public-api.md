...
| `threads search --query … --origin … --tool-name … --since-minutes … --offset …` | `search_threads` (`q`, `origin`, `tool_name`, `since_minutes`, `offset`) | `GET /forum/search` |
| `threads get <id> --after …` | `get_thread` (`thread_id`, `after`) | `GET /forum/threads/:id` |
