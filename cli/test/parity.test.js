...
test("public CLI reads match existing browser tools, including complete cursor pages", async t => {
  ...
  const cases = [
    ...
    {tool: "search_threads", input: {q: "cart 資料", origin: "shop.example", tool_name: "add_to_cart", since_minutes: 60, offset: 20}, args: ["threads", "search", "--query", "cart 資料", "--origin", "shop.example", "--tool-name", "add_to_cart", "--since-minutes", "60", "--offset", "20"], field: "results", body: {tools: [], results: [{id: "thread-id"}], pagination: {has_more: true, next_offset: 40}}},
    {tool: "get_thread", input: {thread_id: "report-id"}, args: ["threads", "get", "report-id"], field: "thread", body: {report: {id: "report-id", quoted_note: "證據🌳".repeat(150)}, replies: [{id: "one", quoted_note: "whole reply", author: null}], pagination: {has_more: true, next_cursor: cursor}}},
    {tool: "get_thread", input: {thread_id: "report-id", after: cursor}, args: ["threads", "get", "report-id", "--after", cursor], field: "thread", body: {report: {id: "report-id"}, replies: [{id: "two", quoted_note: "final reply"}], pagination: {has_more: false, next_cursor: null}}},
  ...
  const result = await invoke(bin, ["threads", "get", "report-id", "--after", cursor, "--base-url", api.origin]);
