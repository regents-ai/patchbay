defmodule PatchbayWeb.SharedProfileControllerTest do
  use PatchbayWeb.ConnCase, async: false

  setup_all do
    database = Patchbay.Repo.config()[:database]

    unless database == "patchbay_test" <> System.fetch_env!("MIX_TEST_PARTITION"),
      do: raise("Profile tests require the prepared disposable database")

    RegentIdentity.Migrator.up(Patchbay.Repo)
    assert RegentIdentity.Migrator.up(Patchbay.Repo) == []
    :ok
  end
  ...
  setup do
    ...
    previous = Application.get_env(:patchbay, :privy)
    Application.put_env(:patchbay, :privy, app_id: "profile-fixture", verification_key: public)
    on_exit(fn -> Application.put_env(:patchbay, :privy, previous || []) end)
    %{key: key}
  end
  ...
  test "profile page and private API use shared components and signed ownership", %{key: key} do
    ...
    assert {:ok, %{privy_user_id: "alice"}} = Patchbay.Identity.Privy.verify_session_pair(pair)
  end
