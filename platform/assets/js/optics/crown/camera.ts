...
/** Extra margin so the smaller crown stays a corner mark, not a hero fill. */
export const CROWN_FRAME_MARGIN = 0.22
...
/** Parks the crown in the upper-left, leaving the header and feed clear. */
export function crownFrameOffsetNdc(
  aspect: number,
  centeredViewProjection?: Float32Array,
): readonly [number, number] {
  const landscapeProgress = clamp((aspect - 1.05) / 0.35, 0, 1)
  const desiredX = -0.58 - 0.08 * landscapeProgress
  const desiredY = 0.42
  ...
  const usableNdc = 1 - CROWN_FRAME_MARGIN * 2
  return [
    Math.max(desiredX, -usableNdc - bounds.minX),
    Math.min(desiredY, usableNdc - bounds.maxY),
  ]
}
...
function projectedCrownBounds(matrix: Float32Array) {
  ...
  for (const [x, y, z] of CROWN_AABB_CORNERS) {
    ...
    minX = Math.min(minX, clipX / clipW)
    maxY = Math.max(maxY, clipY / clipW)
  }
  return {minX, maxY}
}
